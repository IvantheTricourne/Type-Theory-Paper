...
We now can write a simple proof that shows the union of \(\top\) and \(\top\)
is equivalent to the Boolean type, that is {\tt ⊤ + ⊤} = {\tt 𝔹}. Alternatively,
since \(\top\) is occasionally written as 1 and {\tt 𝔹} as 2, we name our proof
as follows:
\begin{center}
\begin{minipage}{0.9\textwidth}
\begin{code}
...
given Boolean (i.e., True or False). This works out quite simply since the
type {\tt ⊤ + ⊤} only has two possible terms, that is {\tt (inl *)} and
{\tt (inr *)}. Thus, we can write the proof as follows:
\begin{center}
\begin{minipage}{0.9\textwidth}
\begin{code}
...
\end{minipage}
\end{center}
To clarify, this form of equivalence is called ``judmental equality''. We
go over the three different forms of equivalence in the following section.
